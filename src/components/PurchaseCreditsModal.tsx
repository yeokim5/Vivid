...
const PurchaseForm: React.FC<Omit<PurchaseCreditsModalProps, "isOpen">> = ({
  ...
  useEffect(() => {
    ...
    const handleStripeError = (event: ErrorEvent) => {
      if (
        (event.message && event.message.includes("Stripe")) ||
  ...
  const handlePurchase = async () => {
    ...
    try {
      ...
      const intentResponse = await fetch(
        `${API_URL}/credits/create-payment-intent`,
        {
          method: "POST",
          headers: {
      ...
      if (paymentIntent.status === "succeeded") {
        ...
        const response = await fetch(`${API_URL}/credits/purchase`, {
          ...
          body: JSON.stringify({
            credits,
            amount: 4.99, // Fixed amount for 10 credits
          }),
        });
      ...
      {purchaseSuccess ? (
        ...
        <>
          ...
          <p className="credits-subtitle">
            Unlock the power to create more vivid essays
          </p>

          <div className="credits-promo-message">
            <span className="highlight">Special Offer:</span> 10 credits for
            only $4.99!
          </div>
          ...
          <div className="single-package">
            <div className="package-content">
              ...
              <p className="per-credit">Just $0.50 per credit</p>
            </div>
          </div>
          ...
          >
            {isProcessing ? "Processing..." : `Purchase 10 Credits for $4.99`}
          </button>

          <p className="terms-note">
            By purchasing credits, you agree to our{" "}
            <a href="#terms">Terms of Service</a>. Payments are processed
